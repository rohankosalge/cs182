...
class AttentionQKV(nn.Module):
    ...
    def forward(self, queries, keys, values, mask=None):
        ...
        keys_swapped = keys.swapaxes(-1, -2)
        similarity = (queries @ keys_swapped) / (key_dim ** 0.5) # Compute the similarity according to the QKV formula
        ...
        softmax = nn.Softmax(dim=-1) # Setting dim to -1 is important here, otherwise it defaults to None.
        weights = softmax(masked_similarity) # Turn the similarity into a normalized output. Remember that the last dim contains the features
        output = weights @ values # Obtain the output
        ####################################  END OF YOUR CODE  ##################################
...
class MultiHeadProjection(nn.Module):
    ...
    def _split_heads(self, tensor):
        ...
        # feature_size = n_heads * new_feature_size.
        # so new_feature_size = feature_size / n_heads
        ...
        # Ok, wow. "Outer" doesn't literally mean the "outer" edge dims. They are the first k dims!
        tensor = th.permute(tensor, (0, 2, 1, 3))
        return tensor
        ##########################################################################################
    ...
    def _combine_heads(self, tensor):
        ...
        # Before: (batch_size, self.n_heads, tensorlen, feature_size)
        tensor = th.permute(tensor, (0, 2, 1, 3))
        # After: (batch_size, tensorlen, self.n_heads, feature_size)
        batch_size, tensorlen = tensor.shape[0], tensor.shape[1]
